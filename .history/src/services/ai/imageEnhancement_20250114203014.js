...
export const enhanceImage = async (imageElement) => {
    try {
        ...
        const response = await fetch('https://us-central1-projekt-galeria-sypniewski-m.cloudfunctions.net/enhanceImage', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Server enhancement failed: ${response.status} ${response.statusText}`);
        }

        // Otrzymujemy bezpośrednio przetworzony obraz
...
const getEnhancementEndpoint = () => {
    // Zawsze używamy produkcyjnego endpointu
    return 'https://us-central1-projekt-galeria-sypniewski-m.cloudfunctions.net/enhanceImage';
};