...
export const enhanceImage = async (imageElement) => {
    try {
        ...
        // Upewnij się, że obraz jest załadowany
        ...
        const response = await fetch(getEnhancementEndpoint(), {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Server enhancement failed: ${response.status} ${response.statusText} - ${errorText}`);
        }
...
// Funkcja pomocnicza do pobierania ulepszonego obrazu
export const downloadEnhancedImage = (metadata, filename = 'enhanced-image.jpg') => {
...
const getEnhancementEndpoint = () => {
    const endpoints = {
        development: 'http://localhost:5001/projekt-galeria-sypniewski-m/us-central1/enhanceImage',
        production: 'https://us-central1-projekt-galeria-sypniewski-m.cloudfunctions.net/enhanceImage',
        staging: 'https://sypniewskimarcin.github.io/api/enhanceImage',
        preview: 'https://projekt-galeria-sypniewski-m.web.app/api/enhanceImage'
    };

    // Sprawdź czy URL zawiera którąś z domen
    const currentOrigin = window.location.origin;
    if (currentOrigin.includes('localhost')) {
        return endpoints.development;
    } else if (currentOrigin.includes('sypniewskimarcin.github.io')) {
        return endpoints.staging;
    } else if (currentOrigin.includes('projekt-galeria-sypniewski-m.web.app')) {
        return endpoints.preview;
    }
