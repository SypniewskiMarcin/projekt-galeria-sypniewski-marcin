...
const ImageEditor = ({ images, onClose, onSave, isOpen }) => {
  ...
  const handleSuperResolution = async () => {
    try {
      ...
      await new Promise((resolve, reject) => {
        img.onload = resolve;
        img.onerror = (e) => reject(new Error(`Nie można załadować obrazu: ${e.message}`));
        img.src = currentImage.url;
      });

      const result = await enhanceImage(img);
      console.log('Otrzymano wynik ulepszania:', result);
      ...
      setAlertMessage('Ulepszanie AI zakończone pomyślnie!');
  ...
  const handleSaveAndDownload = async () => {
    try {
      ...
      if (currentImage.isAIEnhanced && currentImage.editedBlob) {
        ...
        const filename = `enhanced_${path.basename(currentImage.url)}`;
