...
// Konfiguracja CORS dla Express i Firebase Functions
const corsOptions = {
  origin: [
    "http://localhost:3000",
...
// Middleware CORS dla Firebase Functions
...
const loadEsrganModel = async () => {
  if (!isModelLoaded) {
    try {
      // Używamy modelu z TensorFlow Hub
...
exports.enhanceImage = onRequest({
  ...
  try {
    logger.info("1. Rozpoczęcie obsługi żądania enhanceImage", {
    ...
    logger.info("2. Konfiguracja CORS dla origin:", origin);

    if (corsOptions.origin.includes(origin)) {
      response.set("Access-Control-Allow-Origin", origin);
      logger.info("3. Ustawiono dozwolony origin:", origin);
    } else {
      response.set("Access-Control-Allow-Origin", corsOptions.origin[0]);
      logger.warn("3. Origin niedozwolony, ustawiono domyślny:", origin);
    }

    response.set("Access-Control-Allow-Methods", corsOptions.methods.join(", "));
    response.set("Access-Control-Allow-Headers", corsOptions.allowedHeaders.join(", "));
    response.set("Access-Control-Expose-Headers", corsOptions.exposedHeaders.join(", "));
    response.set("Access-Control-Allow-Credentials", "true");
    response.set("Access-Control-Max-Age", corsOptions.maxAge.toString());

    logger.info("4. Ustawiono wszystkie nagłówki CORS");

    if (request.method === "OPTIONS") {
      logger.info("5. Obsługa żądania OPTIONS - zwracam 204");
      return response.status(204).send("");
    }

    // Weryfikacja tokenu uwierzytelniającego
    const authHeader = request.headers.authorization;
    logger.info("6. Weryfikacja autoryzacji:", {
      hasAuthHeader: !!authHeader,
      startsWithBearer: authHeader?.startsWith("Bearer "),
    });

    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      logger.error("7. Brak lub nieprawidłowy token autoryzacji");
    ...
    try {
      ...
      logger.info("8. Token zweryfikowany pomyślnie:", {
        uid: decodedToken.uid,
        email: decodedToken.email,
      });
    } catch (error) {
      logger.error("8. Błąd weryfikacji tokenu:", error);
      return response.status(401).json({
        error: "Unauthorized",
    ...
    logger.info("9. Otrzymano URL obrazu:", imageUrl);

    if (!imageUrl) {
      logger.error("10. Brak URL obrazu w żądaniu");
    ...
    // Pobierz obraz
    logger.info("11. Próba pobrania obrazu");
    const imageResponse = await fetch(imageUrl);
    if (!imageResponse.ok) {
      logger.error("12. Błąd pobierania obrazu:", {
    ...
    logger.info("13. Obraz pobrany pomyślnie, rozmiar:", imageBuffer.length);

    // Przetwarzanie obrazu
    try {
      logger.info("14. Rozpoczęcie przetwarzania AI");
      const enhancedImageBuffer = await enhanceImageWithEsrgan(imageBuffer);
      logger.info("15. Przetwarzanie AI zakończone pomyślnie");

      // Pobierz nazwę pliku z URL
      const fileName = imageUrl.split("/").pop().split("?")[0];

      // Inicjalizacja bucket
      const bucket = admin.storage().bucket();

      // Zapisz przetworzony obraz w Firebase Storage
      const enhancedImagePath = `enhanced/${fileName}`;
      await bucket.file(enhancedImagePath).save(enhancedImageBuffer, {
        metadata: {
      ...
      const enhancedUrl = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodeURIComponent(enhancedImagePath)}?alt=media`;
      ...
      logger.error("15. Błąd przetwarzania AI:", processingError);
      throw processingError;
    }
  } catch (error) {
    logger.error("16. Błąd krytyczny:", {
      message: error.message,
      stack: error.stack,
    });
  ...
  return corsMiddleware(request, response, async () => {
    try {
      ...
      const processWithRetry = async (retryCount = 0) => {
        try {
          // Pobierz dane albumu przed sprawdzeniem watermark
          ...
          // Early return dla przekroczonej liczby prób
          if (retryCount >= maxRetries) {
            throw new Error(`Maximum retry attempts (${maxRetries}) exceeded`);
          }

          // Early return dla wyłączonego watermark
...
// Eksportujemy aplikację Express jako funkcję Firebase
