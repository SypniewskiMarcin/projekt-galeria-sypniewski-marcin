...
exports.enhanceImage = onRequest({
  cors: {
    origin: ['http://localhost:3000', 'https://sypniewskimarcin.github.io'],
  ...
  try {
    // Zawsze ustawiaj nagłówki CORS dla każdego żądania
    const origin = request.headers.origin;
    if (origin === 'http://localhost:3000' || origin === 'https://sypniewskimarcin.github.io') {
      response.set('Access-Control-Allow-Origin', origin);
      response.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
      response.set('Access-Control-Allow-Headers', 'Content-Type, Authorization, Accept, Origin');
      response.set('Access-Control-Max-Age', '3600');
    }
