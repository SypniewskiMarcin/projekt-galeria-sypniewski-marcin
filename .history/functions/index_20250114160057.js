...
const enhanceImageWithEsrgan = async (imageBuffer) => {
  try {
    logger.info("Rozpoczęcie przetwarzania obrazu");

    // Używamy sharp do podstawowego ulepszenia obrazu
    const enhancedImage = await sharp(imageBuffer)
      // Zwiększamy rozmiar obrazu 4x
      .resize({
        width: null,
        height: null,
      ...
      // Dostosowujemy kontrast i jasność
      .modulate({
        brightness: 1.1,
        saturation: 1.2
      })
      // Redukujemy szum
      .median(1)
      // Kompresujemy do JPEG z wysoką jakością
      .jpeg({
        quality: 95,
