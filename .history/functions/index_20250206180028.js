...
// Cache dla modelu ESRGAN i jego konfiguracja
let esrganModel = null;
const ESRGAN_MODEL_URL = "https://tfhub.dev/captain-pool/esrgan-tf2/1";
...
// Funkcja pomocnicza do przetwarzania obrazu przez ESRGAN
const processWithESRGAN = async (imageBuffer) => {
  try {
    ...
    // Przetwórz przez model
    logger.info("Przetwarzanie przez ESRGAN");
    ...
    logger.error("Błąd w processWithESRGAN:", error);
    throw error;
  }
};
 ...
 * Wykonuje detekcję obiektów i ulepszanie ESRGAN
 */
exports.processImage = onRequest({
  cors: {
    origin: true, // Pozwól na wszystkie pochodzenia podczas developmentu
  ...
  try {
    ...
    if (request.method === 'OPTIONS') {
      ...
      response.set('Access-Control-Allow-Headers', 'Content-Type, Authorization, Accept, Origin, X-Requested-With');
    ...
    logger.info("Rozpoczynam przetwarzanie obrazu - szczegóły requestu:", {
      headers: request.headers,
    ...
    if (!imageUrl || !albumId) {
      logger.error("Brak wymaganych parametrów:", { imageUrl, albumId });
      return response.status(400).json({
        error: "Bad Request",
    ...
    // Pobierz obraz
    logger.info("Pobieranie obrazu z URL:", imageUrl);
    const imageResponse = await nodeFetch(imageUrl);
    if (!imageResponse.ok) {
      logger.error("Błąd pobierania obrazu:", {
    ...
    // Przetwórz obraz przez ESRGAN
    logger.info("Rozpoczynam ulepszanie obrazu przez ESRGAN");
    const enhancedImage = await processWithESRGAN(imageBuffer);
    ...
    // Przetwórz obraz używając sharp dla finalnych poprawek
...
/**
 * Funkcja do ulepszania obrazu przez ESRGAN
 */
exports.enhanceImage = onRequest({
  ...
  return cors(request, response, async () => {
    try {
      ...
      logger.info("Otrzymano żądanie ulepszenia obrazu:", {
        headers: request.headers,
        origin: request.headers.origin,
        method: request.method,
      });
